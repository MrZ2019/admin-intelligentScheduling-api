...
exports.findUserList = function(query) {
    ...
    let p = new Promise(async (resolve)=> {
        let q = await dbase.collection("user").find({}).limit(query.size).skip(query.page * query.size)
...
exports.deleteUser =  function(query) {
    ...
    let p = new Promise(async (resolve)=> {
        ...
        for (let index = 0; index < ids.length; index++) {
            ...
            let res = await dbase.collection("user").deleteOne({
                _id: ObjectId(element)
            })
            ...
            console.log(count + " 条文档被删除");
            resolve(count)        
        // dbase.collection("user").deleteMany(deleteStr, function(err, res) {
        //     if (err) throw err;
